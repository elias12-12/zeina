...
export class SalesRepository {
   ...
   async create({ user_id }) {
        const sql = `
            ...
            VALUES ($1, 0, 0, 0, 0)
            RETURNING sale_id, user_id, sale_date, subtotal, discount_percentage, discount_amount, total_amount;
        `;
    ...
    async updateTotalAmount(sale_id, total_amount) {
        const sql = `
            ...
            SET total_amount = $1
            WHERE sale_id = $2
            RETURNING sale_id, user_id, subtotal, discount_percentage, discount_amount, total_amount, sale_date;
        `;
    ...
    async updateWithDiscount(sale_id, subtotal, discount_percentage) {
      ...
      async findAll() {
        const sql = `SELECT sale_id, user_id, subtotal, discount_percentage, discount_amount, total_amount, sale_date FROM sales ORDER BY sale_id DESC;`;
        const { rows } = await pool.query(sql);
        return rows.map(r => new Sales(r));
    }

  async findById(sale_id) {
        const sql = `SELECT sale_id, user_id, subtotal, discount_percentage, discount_amount, total_amount, sale_date FROM sales WHERE sale_id = $1;`;
        const { rows } = await pool.query(sql, [sale_id]);
        return rows[0] ? new Sales(rows[0]) : null;
    }

      async findByCustomer(user_id) {
        const sql = `SELECT sale_id, user_id, subtotal, discount_percentage, discount_amount, total_amount, sale_date FROM sales WHERE user_id = $1 ORDER BY sale_id DESC;`;
        const { rows } = await pool.query(sql, [user_id]);
        return rows.map(r => new Sales(r));
    }
